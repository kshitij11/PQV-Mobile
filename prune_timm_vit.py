...
def parse_args():
    ...
    parser.add_argument('--pruning_ratio', default=0.25, type=float, help='prune ratio')
    parser.add_argument('--bottleneck', default=False, action='store_true', help='bottleneck or uniform')
    parser.add_argument('--pruning_type', default='taylor', type=str, help='pruning type', choices=['random', 'taylor', 'l2', 'l1', 'hessian'])
    ...
    parser.add_argument('--save_as', default='pruned/model_taylor_0.25.pth', type=str, help='save the pruned model')
...
def prepare_imagenet_train(imagenet_root, train_batch_size=64, val_batch_size=128, num_workers=4, use_imagenet_mean_std=False):
...
def prepare_imagenet_val(imagenet_root, train_batch_size=64, val_batch_size=128, num_workers=4, use_imagenet_mean_std=False):
    """The imagenet_root should contain train and val folders.
    """
...
def main():
    ...
    if args.test_accuracy:
        val_loader = prepare_imagenet_val(args.data_path, train_batch_size=args.train_batch_size, val_batch_size=args.val_batch_size, use_imagenet_mean_std=args.use_imagenet_mean_std)
    if args.pruning_type in ['taylor', 'hessian']:
        train_loader = prepare_imagenet_train(args.data_path, train_batch_size=args.train_batch_size, val_batch_size=args.val_batch_size, use_imagenet_mean_std=args.use_imagenet_mean_std)

    # Load the model
    model = torch.hub.load('facebookresearch/deit:main', 'deit_base_patch16_224', pretrained=True).to(device)
    input_size = [3, 224, 224]
    example_inputs = torch.randn(1, *input_size).cuda()
